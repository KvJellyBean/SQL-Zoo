...
SELECT F.num, F.company, F.name, S.num, S.company
FROM 
...
ON (a.company=b.company AND a.num=b.num)
  JOIN stops stopa ON (a.stop=stopa.id)
  JOIN stops stopb ON (b.stop=stopb.id)
WHERE stopa.name='Craiglockhart' AND stopb.name <> 'Craiglockhart') as F
JOIN
(SELECT DISTINCT b.num, b.company, stopc.name
FROM route b JOIN route c
ON (b.company=c.company AND b.num=c.num)
  JOIN stops stopb ON (b.stop=stopb.id)
  JOIN stops stopc ON (c.stop=stopc.id)
WHERE stopb.name='Lochend' AND stopc.name <> 'Lochend') as S
ON (F.name = S.name)
ORDER BY F.num, F.name, S.num
